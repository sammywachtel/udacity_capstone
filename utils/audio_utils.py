...
def create_spectrogram_parallelized(audio, sample_rate, audioname, directory,
    ...
    ret_val = batch_num * batch_size + batch_i_num+1, batch_num+1, num_samples
...
def write_spectograms_parallelized(tfrecord_file_name, directory, batch_size = 50, mel=False, 
                                   overwrite=False, regex_filter=None):
    ...
    with tf.Graph().as_default():
        ...
        if regex_filter is not None:
            dataset = dataset.filter(lambda d: tf.strings.regex_full_match(d['note_str'],regex_filter))

        #set to different number to take fewer samples for testing, but don't use this to limit model recordset
        num_tfrecords = general_utils.get_tfdataset_count(dataset)
